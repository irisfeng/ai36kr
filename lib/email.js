...
function shell({ title, body }) {
  ...
  <div style="max-width:560px;margin:0 auto;padding:28px 20px;font-family:-apple-system,'PingFang SC','Noto Sans SC',sans-serif;color:#191813;">
    <div style="border-bottom:4px double #191813;padding-bottom:14px;display:flex;align-items:center;gap:10px;">
      ...
      <div style="margin-left:auto;font-family:Menlo,monospace;font-size:10px;color:#8B8574;">${title}</div>
    </div>
    ${body}
...
export function confirmEmailHtml({ confirmUrl }) {
  return shell({
    ...
    <p style="margin:22px 0;"><a href="${confirmUrl}" style="display:inline-block;background:#C23B22;color:#FCFBF7;padding:11px 26px;text-decoration:none;font-size:14px;font-weight:600;">确认订阅 →</a></p>
    <p style="font-size:12px;color:#8B8574;">如果按钮无效，复制此链接到浏览器：<br>${confirmUrl}</p>`,
  });
}

export function dailyEmailHtml({ dateStr, words, top3, groups, total, unsubUrl, dailyUrl }) {
  const wordStr = words.slice(0, 8).map((w) => `${w.word}×${w.count}`).join(' · ');
  const top = top3.map((p, i) => `
    <div style="border:1px solid #DCD7C7;padding:12px 14px;margin:8px 0;background:#FCFBF7;">
      ...
      <a href="${SITE}/post/${p.id}" style="color:#191813;text-decoration:none;font-family:'Songti SC',serif;font-weight:900;font-size:15px;margin-left:8px;">${(p.title_zh || p.title)}</a>
      <div style="font-size:11px;color:#8B8574;margin-top:4px;font-family:Menlo,monospace;">${p.source} · ▲${p.up - p.down}</div>
    </div>`).join('');
  const cats = groups.map(([cat, items]) => {
    const lines = items.slice(0, 5).map((p) =>
      `<div style="padding:7px 0;border-bottom:1px solid #EDEAE0;"><a href="${SITE}/post/${p.id}" style="color:#4B463A;text-decoration:none;font-size:13px;">${(p.title_zh || p.title)}</a></div>`
    ).join('');
    return `<h3 style="font-family:'Songti SC',serif;font-size:15px;margin:18px 0 6px;"><span style="color:#C23B22;">■</span> ${cat} <span style="font-size:11px;color:#8B8574;font-weight:400;">${items.length}</span></h3>${lines}`;
  }).join('');

  return shell({
    title: dateStr,
    body: `
    <p style="font-family:Menlo,monospace;font-size:11px;color:#8B8574;margin:16px 0 4px;">${dateStr} · 24 小时收录 ${total} 条</p>
    ...
    ${top}
    ${cats}
    <p style="margin:22px 0;"><a href="${dailyUrl}" style="display:inline-block;border:1px solid #191813;color:#191813;padding:10px 24px;text-decoration:none;font-size:13px;">打开完整今日一页 →</a></p>
    <p style="font-size:11px;color:#8B8574;"><a href="${unsubUrl}" style="color:#8B8574;">退订日报</a></p>`,
  });
}
