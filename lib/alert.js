...
export async function checkSourceAlerts(db) {
  ...
  for (const s of rows) {
    ...
    if (!claimed) continue;
    try {
      // 同源去重：已有未关闭的同源告警 issue 则不再新开
      const dup = await fetch(
        `https://api.github.com/search/issues?q=${encodeURIComponent(`repo:${REPO} state:open label:source-alert in:title "${s.name}"`)}`,
        { headers: { Authorization: `Bearer ${token}`, Accept: 'application/vnd.github+json', 'User-Agent': 'TideWire-Alert/1.0' } }
      );
      if (dup.ok) {
        const found = await dup.json();
        if ((found.total_count || 0) > 0) continue;
      }
