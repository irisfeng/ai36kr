...
function applyVote(db, target, id, upDelta, downDelta) {
  if (target.hasDown) {
    ...
    // 票数变化即时反映到预计算热度分（列表 SQL 直排依赖它）；
    // 测试 fixture 等精简库无 hot_score/ext_score 列时静默跳过
    try {
      db.exec(`UPDATE posts SET hot_score =
        (up - down + sqrt(COALESCE(ext_score, 0)))
        / ((MAX((julianday('now') - julianday(created_at)) * 24, 0) + 2)
          * sqrt(MAX((julianday('now') - julianday(created_at)) * 24, 0) + 2))
