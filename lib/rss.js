...
async function fetchFeed(feed, sigs = null) {
  ...
  for (const item of (parsed.items || []).slice(0, MAX_ITEMS_PER_FEED)) {
    ...
    const r = stmt.run(title, normalizeTitle(title), feed.name, category, summary || title, content, feed.deep ? 1 : 0, toIsoDate(item), url, feed.home, extractFeedImage(item), hnPoints);
...
async function refreshGroup(feeds, fetcher) {
  ...
  return { totalNew, okCount, backoffCount };
}
...
async function refreshAll() {
  ...
  console.log(
    `[听潮] 聚合完成：文章 +${posts.totalNew} / 快讯 +${flashes.totalNew} / 新品 +${products.totalNew}，${okCount}/${feedCount} 个源在线${backoffCount ? `，${backoffCount} 个退避中` : ''}`
  );
