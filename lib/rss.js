...
import { titleTokens, findDuplicate, loadRecentSignatures } from './dedupe.js';
...
export const FEEDS = [
  ...
  { name: '36氪', url: 'https://www.36kr.com/feed', home: 'https://36kr.com', filterAI: true },
...
export const FLASH_FEEDS = [
  ...
  // 36氪快讯：RSSHub 实例不稳，主备三条链
  { name: '36氪快讯', url: 'https://rsshub.bestblogs.dev/36kr/newsflashes', home: 'https://36kr.com/newsflashes',
    fallbacks: ['https://rsshub.rssforever.com/36kr/newsflashes', 'https://rsshub.pseudoyu.com/36kr/newsflashes'] },
...
const insertPostStmt = () => db.prepare(
  `INSERT OR IGNORE INTO posts
     (title, title_norm, source, category, summary, content, is_deep, up, down, created_at, url, is_external, source_home, image_url, ext_score)
   VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, ?, ?, 1, ?, ?, ?)`
);
...
async function fetchFeed(feed, sigs = null) {
  ...
  for (const item of (parsed.items || []).slice(0, MAX_ITEMS_PER_FEED)) {
    ...
    // 跨源模糊去重：与近 7 天词集 Jaccard ≥ 0.5 视为同一事件（精确去重已由 title_norm 索引兜底）
    const tokens = titleTokens(title);
    if (sigs && findDuplicate(sigs, tokens, 0.5) !== null) continue;
    const r = runStmt(stmt, title, normalizeTitle(title), feed.name, category, summary || title, content, feed.deep ? 1 : 0, toIsoDate(item), url, feed.home, extractFeedImage(item), hnPoints);
    inserted += Number(r.changes);
    if (r.changes > 0 && sigs) sigs.set(Number(r.lastInsertRowid), tokens);
  }
  return inserted;
}
...
async function fetchFlashFeed(feed) {
  ...
  for (const item of (parsed.items || []).slice(0, MAX_ITEMS_PER_FEED)) {
    ...
    const r = runStmt(stmt, content, tagFlash(`${title} ${desc}`), toIsoDate(item), url, feed.name);
    inserted += Number(r.changes);
  }
  return inserted;
}
...
async function fetchProductFeed(feed) {
  ...
  for (const item of (parsed.items || []).slice(0, 30)) {
    ...
    const r = runStmt(stmt, name, tagline, description, feed.name, toIsoDate(item), url, extractFeedImage(item));
    inserted += Number(r.changes);
  }
  return inserted;
}
...
async function refreshAll() {
  // 近 7 天标题词集：本轮所有源共享，用于跨源模糊去重（含本轮新入库的）
  const sigs = loadRecentSignatures(db);
  const [posts, flashes, products] = await Promise.all([
    refreshGroup(FEEDS, (feed) => fetchFeed(feed, sigs)),
  ...
  const backoffCount = (posts.backoffCount || 0) + (flashes.backoffCount || 0) + (products.backoffCount || 0);
  const dbErrors = (posts.dbErrors || 0) + (flashes.dbErrors || 0) + (products.dbErrors || 0);
  const feedCount = FEEDS.length + FLASH_FEEDS.length + PRODUCT_FEEDS.length;
  lastRefreshAt = Date.now();
  console.log(
    `[听潮] 聚合完成：文章 +${posts.totalNew} / 快讯 +${flashes.totalNew} / 新品 +${products.totalNew}，${okCount}/${feedCount} 个源在线${backoffCount ? `，${backoffCount} 个退避中` : ''}${dbErrors ? `，⚠️ ${dbErrors} 个 DB 写失败` : ''}`
  );
  // 旧内容清理：聚合文章留 30 天（深度长文留 90 天）、快讯留 7 天；
  // 社区投稿（is_external=0）与有评论/投票/反应的帖一律保留；孤儿投票/反应一并清
  try {
    ...
    const prunedPosts = db.prepare(
      `DELETE FROM posts WHERE is_external = 1
         ...
         AND id NOT IN (SELECT DISTINCT target_id FROM votes WHERE target_type = 'post')
  ...
  // og 回填：CI 主力 60/轮，线上限量 6/轮防超时
  const ogLimit = process.env.VERCEL ? 6 : 60;
  try {
  ...
  try {
    ...
    console.warn('[听潮] 摘要翻译异常:', e?.message || e);
  }
  // 存量译文自动校正：词表新增 fix 词条下一轮即生效，无需人工跑脚本
  try {
    const fixed = await refixTitles(db);
    if (fixed) console.log(`[听潮] 存量译文校正 ${fixed} 条`);
  } catch (e) {
    console.warn('[听潮] 存量校正异常:', e?.message || e);
  }
  // 热度分预计算：等价 (up-down+√ext)/(age+2)^1.5（x·√x 替代不存在的 pow）
  // 一条 SQL 全表重算，列表查询直接 ORDER BY hot_score，零 Node 排序
  try {
    db.exec(`UPDATE posts SET hot_score =   (up - down + sqrt(COALESCE(ext_score, 0)))   / ((MAX((julianday('now') - julianday(created_at)) * 24, 0) + 2)     * sqrt(MAX((julianday('now') - julianday(created_at)) * 24, 0) + 2))`);
  } catch (e) {
    console.warn('[听潮] 热度重算异常:', e?.message || e);
  }
  // 源健康告警：连续失败自动开 GitHub Issue（fire-and-forget，不拖慢聚合）
  try {
    const alerted = await checkSourceAlerts(db);
    if (alerted) console.warn(`[听潮] 新建 ${alerted} 个源告警 issue`);
  } catch (e) {
    console.warn('[听潮] 源告警异常:', e?.message || e);
  }
  return { totalNew, okCount, dbErrors };
}
