...
import db from './db';
import { classifyPost, isAiRelated } from './classify';
...
const insertPostStmt = () => db.prepare(
  `INSERT OR IGNORE INTO posts
     (title, source, category, summary, content, is_deep, up, down, created_at, url, is_external, source_home, image_url)
   VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?, ?, 1, ?, ?)`
);
...
async function fetchFeed(feed) {
  ...
  for (const item of (parsed.items || []).slice(0, MAX_ITEMS_PER_FEED)) {
    ...
    const r = stmt.run(title, feed.name, category, summary || title, content, feed.deep ? 1 : 0, toIsoDate(item), url, feed.home, extractFeedImage(item));
...
async function refreshGroup(feeds, fetcher) {
  ...
  results.forEach((r, i) => {
    if (r.status === 'fulfilled') {
      ...
      console.warn(`[AI氪] 源抓取失败 ${feeds[i].name}: ${msg}`);
    }
  });
...
async function refreshAll() {
  ...
  console.log(
    `[AI氪] 聚合完成：文章 +${posts.totalNew} / 快讯 +${flashes.totalNew} / 新品 +${products.totalNew}，${okCount}/${feedCount} 个源在线`
  );
  // 缩略图回填：RSS 无图的条目抓原文 og:image（首轮大批量，之后每轮限量）
  const firstRun = !globalThis.__aikrImgBackfilled;
  globalThis.__aikrImgBackfilled = true;
  try {
    const postImgs = await backfillImages('posts', 'id', firstRun ? 60 : 12);
    const prodImgs = await backfillImages('products', 'id', firstRun ? 20 : 6);
    if (postImgs || prodImgs) console.log(`[AI氪] 缩略图回填：文章 +${postImgs} / 新品 +${prodImgs}`);
  } catch (e) {
    console.warn('[AI氪] 缩略图回填异常:', e?.message || e);
  }
  return { totalNew, okCount };
}
...
export function refresh() {
  if (!refreshing) {
    refreshing = refreshAll()
      .catch((e) => console.warn('[AI氪] 聚合异常:', e?.message || e))
...
export function refreshIfStale(interval = REFRESH_INTERVAL_MS) {
  if (Date.now() - lastRefreshAt < interval || refreshing) return false;
  refresh();
  return true;
}
...
export function startAggregation() {
  ...
  timer.unref?.();
  console.log(`[AI氪] RSS 聚合已启动，每 ${REFRESH_INTERVAL_MS / 60000} 分钟刷新`);
}
