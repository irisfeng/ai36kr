...
// 结果写回 posts.title_zh 永久缓存；翻译失败留 NULL 下轮重试（不标废——
// 失败多为环境性（密钥缺失/网络不可达），标 '' 会永久锁死且随快照扩散）
...
const LLM_PROMPT_SUMMARY = `你是 AI 领域的专业译者，为中文科技媒体翻译新闻摘要。要求：
1. 依据语境语义翻译，通顺自然，不逐字直译；保留原文信息量，不额外发挥。
2. 术语一律保留英文原文：LLM, GPT, Claude, Gemini, DeepSeek, Qwen, Kimi, Sora, Grok, Llama, Mistral, Copilot, Cursor, Transformer, LoRA, MoE, RLHF, RAG, SFT, vLLM, MCP, Hugging Face, LangChain, OpenAI, Anthropic, NVIDIA, CUDA, API, Agent, Kubernetes, PyTorch。
3. 公司与产品名保留原文（OpenAI, Google, Meta, Microsoft, Apple, Amazon, Tesla, xAI, DeepMind）。
4. 译文长度与原文相当（允许 ±30%），不超过 200 字。
5. 只输出 JSON 字符串数组，与输入等长，不要任何解释。`;

async function translateBatchLLM(texts, { kind = 'title' } = {}) {
  const ctrl = new AbortController();
  ...
  try {
    const res = await fetch(ARK_URL, {
      ...
      body: JSON.stringify({
        model: ARK_MODEL,
        messages: [
          { role: 'system', content: kind === 'summary' ? LLM_PROMPT_SUMMARY : LLM_PROMPT },
          { role: 'user', content: `把以下 JSON 数组中的英文${kind === 'summary' ? '摘要' : '标题'}逐条译成中文：\n${JSON.stringify(texts)}` },
        ],
        max_tokens: kind === 'summary' ? 3000 : 1200,
        temperature: 0.2,
      }),
    });
    if (!res.ok) return null;
...
export async function translateBatch(texts, opts) {
  if (process.env.ARK_API_KEY && texts.length) {
    return llmChunk(texts, opts);
  }
  return Promise.all(texts.map((t) => translateText(t)));
}

async function llmChunk(texts, opts) {
  const r = await translateBatchLLM(texts, opts);
  if (r) return r;
  if (texts.length <= 1) return [await translateText(texts[0])];
  const mid = Math.ceil(texts.length / 2);
  const [a, b] = await Promise.all([
    llmChunk(texts.slice(0, mid), opts),
    llmChunk(texts.slice(mid), opts),
  ]);
  return [...a, ...b];
}

// 摘要翻译：英文摘要译中文（LLM 语境批译，5 条/批——摘要较长）
// 先自愈：近 3 天被旧逻辑标废（''）的摘要放回 NULL 重试
export async function translateSummaries(db, limit = 10) {
  db.prepare("UPDATE posts SET summary_zh = NULL WHERE summary_zh = '' AND created_at >= ?")
    .run(new Date(Date.now() - 3 * 86400000).toISOString());
  // 同样按标题首字符预过滤非 CJK，防中文帖占满 300 窗口
  const rows = db
    .prepare("SELECT id, title, summary FROM posts WHERE summary_zh IS NULL AND unicode(substr(title, 1, 1)) < 11904 ORDER BY created_at DESC LIMIT 300")
    .all()
    .filter((r) => r.summary && needsTranslation(r.title) && needsTranslation(r.summary))
    .slice(0, limit);
  if (!rows.length) return 0;
  const stmt = db.prepare('UPDATE posts SET summary_zh = ? WHERE id = ?');
  let done = 0;
  for (let i = 0; i < rows.length; i += 5) {
    const batch = rows.slice(i, i + 5);
    const inputs = batch.map((r) => r.summary.slice(0, 280));
    const translated = await translateBatch(inputs, { kind: 'summary' });
    batch.forEach((r, j) => {
      const zh = translated[j];
      // 失败留 NULL 下轮重试；译文与原文相同才标 '' 不再重试
      stmt.run(zh && zh !== inputs[j] ? zh : zh === inputs[j] ? '' : null, r.id);
      if (zh && zh !== inputs[j]) done++;
    });
  }
  return done;
}
...
// 按新到旧扫：LIMIT 截断时新译文优先（旧译文在它们"新"时已扫过）
export async function refixTitles(db) {
  const since = new Date(Date.now() - 30 * 86400000).toISOString();
  const rows = db
    .prepare("SELECT id, title_zh FROM posts WHERE title_zh IS NOT NULL AND title_zh != '' AND created_at >= ? ORDER BY created_at DESC LIMIT 300")
    .all(since);
...
// 每轮聚合后调用：待译标题限量翻译
// 不设时间窗：NULL 行按新到旧逐轮排空（深度长文留存 90 天，不设窗才不遗漏）
// 失败留 NULL 下轮重试；先自愈：近 3 天被旧逻辑标废（''）的标题放回 NULL
// 注意：中文文章 title_zh 恒为 NULL，会占住排序靠前的位置，故取较大 LIMIT 再过滤
export async function translateTitles(db, limit = 20) {
  db.prepare("UPDATE posts SET title_zh = NULL WHERE title_zh = '' AND created_at >= ?")
    .run(new Date(Date.now() - 3 * 86400000).toISOString());
  // SQL 层预过滤非 CJK 首字符：中文帖 title_zh 恒 NULL，不过滤会把 300 窗口占满、英文帖饿死
  const rows = db
    .prepare("SELECT id, title FROM posts WHERE title_zh IS NULL AND unicode(substr(title, 1, 1)) < 11904 ORDER BY created_at DESC LIMIT 300")
    .all()
  ...
  for (let i = 0; i < rows.length; i += 15) {
    ...
    batch.forEach((r, j) => {
      const zh = translated[j];
      // 失败留 NULL 下轮重试；译文与原文相同（纯专有名词等）才标 '' 不再重试
      stmt.run(zh && zh !== r.title ? zh : zh === r.title ? '' : null, r.id);
