...
if (!isRemote) {
  ...
  const { pending } = db.prepare('SELECT COUNT(*) AS pending FROM posts WHERE title_norm IS NULL').get();
  if (pending > 0) {
    const rows = db.prepare('SELECT id, title FROM posts WHERE title_norm IS NULL').all();
    const seen = new Set();
    const upd = db.prepare('UPDATE posts SET title_norm = ? WHERE id = ?');
    for (const r of rows) {
      const n = norm(r.title);
      if (seen.has(n)) upd.run(null, r.id); // 既有重复：置空避让，数据保留
      else { seen.add(n); upd.run(n, r.id); }
    }
  }
}
...
function hydrateIfEmpty() {
  ...
  const TABLE_COLS = {
    posts: ['id', 'title', 'title_norm', 'source', 'category', 'summary', 'content', 'is_deep', 'up', 'down', 'created_at', 'url', 'is_external', 'source_home', 'image_url'],
