...
if (isRemote) {
  ...
  db = new Database(path.join(dataDir, 'tidewire.db'));
...
function hydrateIfEmpty() {
  ...
  const insert = {
    posts: db.prepare(
      `INSERT OR IGNORE INTO posts (id, title, source, category, summary, content, is_deep, up, down, created_at, url, is_external, source_home, image_url)
       VALUES (@id, @title, @source, @category, @summary, @content, @is_deep, @up, @down, @created_at, @url, @is_external, @source_home, @image_url)`
    ),
    flashes: db.prepare(
      `INSERT OR IGNORE INTO flashes (id, content, tag, up, created_at, url, source)
       VALUES (@id, @content, @tag, @up, @created_at, @url, @source)`
    ),
    products: db.prepare(
      `INSERT OR IGNORE INTO products (id, name, tagline, description, category, up, created_at, url, image_url)
       VALUES (@id, @name, @tagline, @description, @category, @up, @created_at, @url, @image_url)`
    ),
  };
  let n = 0;
  for (const table of ['posts', 'flashes', 'products']) {
    for (const row of snap[table] || []) {
      n += Number(insert[table].run(row).changes);
    }
  }
