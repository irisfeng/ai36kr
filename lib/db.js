...
// 构建期且未配置 Turso 时用本地临时 scratch 库（防 page data collection 拖死）；
// 但配了 Turso 时构建期也必须直连——否则静态首页在构建时用空库预渲染，
// 「空首页」会被边缘缓存 5 分钟（本事故复盘见 P0-1）
const isBuild = process.env.NEXT_PHASE === 'phase-production-build';
const isRemote = !!process.env.TURSO_DATABASE_URL;
const useScratch = isBuild && !isRemote;
...
if (isRemote) {
  ...
  db = new Database(path.join(dataDir, useScratch ? `build-scratch-${process.pid}.db` : 'tidewire.db'));
...
addColumnIfMissing('posts', 'title_norm TEXT');
// 日报失败名单：部分失败时备份时点只补发失败者（digest_log 随表升级）
addColumnIfMissing('digest_log', "failed_emails TEXT NOT NULL DEFAULT '[]'");
// 源连续失败计数（健康告警用）
...
// 热度分预计算（聚合/投票时维护，列表查询直接 SQL 排序，不再全表 Node 排序）
addColumnIfMissing('posts', 'hot_score REAL NOT NULL DEFAULT 0');
if (!isRemote) {
