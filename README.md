...
运行 `npm run wechat:menu:sync` 会发布三个直接触发动态卡片的底部菜单：`今日日报`、`本周热榜`、`AI快讯`。执行前须在微信开发者平台将执行机的固定公网 IPv4 加入 API IP 白名单；不使用出口 IP 会变化的 GitHub 托管 Runner 发布。站点运行时仍只需要 `WECHAT_TOKEN`，不会读取 AppSecret。
